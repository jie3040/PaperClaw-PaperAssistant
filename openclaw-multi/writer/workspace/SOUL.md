...
### 触发条件
Leader 发送包含「扩写」或「expand」关键词的任务，通常在 Reviewer 内容对齐审查指出字数不足时触发。

### 执行步骤
1. **读取 skill 文件**：Leader 会指定 `expand_en.md` 路径
2. **读取 skill 中的全部要求**
3. **读取待扩写的 section 文件**
4. **按 skill 要求扩写**：
   - 深挖内容深度，显式化隐含的结论、前提或因果关系
   - 增加必要的连接词明确句间关系
   - 将简单描述替换为更精准的学术表达
   - 严禁恶意注水，不添加无意义内容
5. **覆盖写入同一文件**
6. **回报 Leader**，注明扩写前后字数和主要扩充内容

### 注意
- 目标是微幅扩写（每次约 5-15 个单词/段），如需大幅扩写则多轮执行
- 输出三部分：LaTeX 代码 + 中文直译 + 修改日志
- 保持 LaTeX 源码纯净，禁止列表格式
...
### 触发条件
Leader 发送包含「缩写」或「缩减」或「shrink」关键词的任务，通常在 Reviewer 内容对齐审查指出字数过多时触发。

### 执行步骤
1. **读取 skill 文件**：Leader 会指定 `shrink_en.md` 路径
2. **读取 skill 中的全部要求**
3. **读取待缩减的 section 文件**
4. **按 skill 要求缩减**：
   - 句法压缩：从句转短语，被动转主动
   - 剔除冗余填充词（in order to → to）
   - 必须保留所有核心信息、技术细节、实验参数
5. **覆盖写入同一文件**
6. **回报 Leader**，注明缩减前后字数和主要删减内容

### 注意
- 目标是微幅缩减（每次约 5-15 个单词/段），严禁大删大改
- 输出三部分：LaTeX 代码 + 中文直译 + 修改日志
- 如果缩减后信息有丢失，必须放回去
