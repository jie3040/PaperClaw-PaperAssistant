...
Recent advancements in 2024--2025 have focused on Transformer architectures and emerging state-space models like Mamba for vibration fault diagnosis. Transformers with advanced positional encodings effectively capture long-range dependencies in vibration spectrograms or tokenized sequences, enabling precise multi-fault identification even under variable operating conditions \cite{chen2024transformer}. Mamba, utilizing selective state spaces, offers linear-time complexity surpassing Transformers' quadratic scaling, making it suitable for long-sequence processing in continuous monitoring applications \cite{li2024mamba}. Evaluations on benchmarks such as Paderborn, HUST, and MFPT datasets report accuracies exceeding 98\%, with notable improvements in noisy environments and multi-fault scenarios \cite{aziz2025mamba}. Hybrid models integrating Mamba with CNN backbones further bolster feature extraction robustness \cite{guo2025hybrid}. Nevertheless, these methods still necessitate domain-specific pretraining to achieve reliable cross-machine generalization.
...
In 2024--2025, advanced VLMs such as GPT-4V and LLaVA-1.6 have been increasingly applied to industrial fault diagnosis, particularly for visual and multimodal inspections. GPT-4V excels in analyzing RGB images of machinery components, providing detailed defect descriptions and localizations; however, it struggles with non-RGB inputs like vibration spectrograms, frequently misinterpreting spectral harmonics and sidebands due to its predominant training on natural photographic images \cite{openai2024gpt4v}. LLaVA-1.6, benefiting from improved visual instruction tuning and larger-scale data, offers better handling of diverse visual formats but is prone to hallucinations, such as confusing fault modulation patterns with unrelated visual artifacts \cite{liu2024llava}. Emerging specialized models like IndustrialVLM and VibraVLM fine-tune these backbones on domain-specific spectrogram-instruction datasets, achieving moderate gains; nonetheless, zero-shot cross-domain performance typically ranges from 75\% to 85\%, revealing persistent challenges in abstract signal comprehension and the necessity for targeted multimodal alignment \cite{zhang2025industrialvlm,liu2025vibravlm}.
