...
\end{enumerate}
The training loop iterates over epochs and batches, encoding vibration-prompt pairs, computing the composite loss, and performing MAML inner/outer parameter updates.
...
Fusion: 8-head cross-attention ($d_{\mathrm{model}}=768$, $d_k=64$):
\begin{equation}
...
(via `wc -w` excluding pure LaTeX commands).
