...
To validate the robustness, scalability, and generalizability of the Diff-LM-GZSL framework, we utilize three prominent industrial fault diagnosis datasets: the Tennessee Eastman Process (TEP), the Hydraulic System dataset, and the Case Western Reserve University (CWRU) bearing dataset. These datasets represent different failure modes (process-based vs. component-based) and signal characteristics (continuous low-frequency vs. high-frequency vibration signals).
