...
\begin{figure}
\centering
\includegraphics[width=\columnwidth]{combined_fig1.png}
\caption{Figure Spectrum Casei}\label{fig:spectrum_caseI}
...
\begin{figure}
\centering
\includegraphics[width=\columnwidth]{combined_fig3.png}
\caption{Figure Confusion Casei}\label{fig:confusion_caseI}
...
\begin{figure}
\centering
\includegraphics[width=\columnwidth]{combined_fig5.png}
\caption{Figure Tsne}\label{fig:tsne}
\label{fig:tsne}
...
In this paper, we proposed CAC-CycleGAN-WGP, a novel data augmentation framework for imbalanced bearing fault diagnosis that combines cycle-consistent generative adversarial networks with Wasserstein gradient penalty and a conditional auxiliary classifier. Experimental results on two bearing datasets (Case I and Case II) demonstrate that our method significantly outperforms state-of-the-art approaches, achieving over 98\% accuracy under severe imbalance ratios while maintaining robustness across various operating conditions.

We summarize the key contributions of this work as follows: (1) We developed a cycle-consistent GAN architecture with Wasserstein gradient penalty that effectively addresses the data imbalance problem in bearing fault diagnosis. (2) The conditional auxiliary classifier ensures class-specific generation, preventing mode collapse and improving the diversity of synthetic samples. (3) The SAE-based quality evaluation provides a reliable metric for assessing the fidelity of generated vibration signals. (4) Comprehensive experiments on benchmark datasets validate the superiority of our method over SMOTE, GAN, WGAN, ACGAN, and other baselines.

However, this study has several limitations that warrant future investigation. First, the proposed framework assumes access to a sufficient number of normal samples for domain translation, which may not be available in all industrial scenarios. Second, the computational cost of training cycle-consistent GANs is higher than traditional oversampling methods, limiting real-time deployment in resource-constrained environments. Third, the current evaluation is limited to two bearing datasets; validation on more diverse machinery fault types is needed.

Future work will explore several promising directions: (1) extending the framework to handle varying operating speeds and load conditions in real-time monitoring systems; (2) integrating advanced domain adaptation techniques to improve transferability across different machine types; (3) optimizing the training process for edge computing deployment; and (4) applying the proposed data augmentation strategy to other rotating machinery fault diagnosis tasks such as gearbox and rotor systems.


\bibliographystyle{IEEEtran}


% === Extracted inline floats ===
\begin{figure*}
\centering
\includegraphics[width=\columnwidth]{fig2.png}
\caption{Figure Architecture}\label{fig:architecture}
...
\caption{Figure Sae}\label{fig:sae}
\label{fig:sae}
\end{figure*}
...
\begin{figure*}
\centering
\includegraphics[width=\columnwidth]{combined_fig2.png}
...
\begin{figure*}
\centering
\includegraphics[width=\columnwidth]{combined_fig4.png}
...
\begin{figure*}
\centering
\includegraphics[width=\columnwidth]{combined_fig6.png}
...
\begin{figure*}
\centering
\includegraphics[width=\columnwidth]{combined_fig7.png}
...
\begin{figure*}
\centering
\includegraphics[width=\columnwidth]{combined_fig8.png}
\caption{Figure Sae Error}
\label{fig:sae_error}
\end{figure*}

% === End extracted floats ===
