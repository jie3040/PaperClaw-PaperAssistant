...
In this paper, we proposed CAC-CycleGAN-WGP, a novel data augmentation framework for imbalanced bearing fault diagnosis that combines cycle-consistent generative adversarial networks with Wasserstein gradient penalty and a conditional auxiliary classifier. Experimental results on two bearing datasets demonstrate that our method significantly outperforms state-of-the-art approaches, achieving over 98\% accuracy under severe imbalance ratios while maintaining robustness across various operating conditions.

Despite the promising results, several limitations of this work should be acknowledged. First, the proposed framework assumes the availability of sufficient normal samples to serve as the source domain for translation, which may not hold in scenarios where both fault and normal data are extremely scarce. Second, the computational cost of training the CAC-CycleGAN-WGP is non-trivial, requiring approximately 120 minutes on a high-performance GPU, which may pose challenges for real-time industrial deployment. Third, while the method demonstrates excellent performance on bearing fault diagnosis, its generalization to other mechanical components such as gears, shafts, or seals remains to be validated. Future research directions include: (1) investigating unsupervised domain adaptation techniques to reduce the reliance on paired training data; (2) exploring lightweight generator architectures for edge deployment in industrial IoT contexts; (3) extending the framework to handle temporal non-stationarity in operational conditions; and (4) integrating physics-informed constraints to ensure physically meaningful generated signals.
