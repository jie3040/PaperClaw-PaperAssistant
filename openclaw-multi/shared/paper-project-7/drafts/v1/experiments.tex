...
Visual comparison of the frequency spectra between real samples and generated synthetic samples provides immediate insight into the model's performance. As shown in Fig. 1 and Fig. 2 (representing Case I and Case II respectively), the synthetic signals successfully capture the characteristic frequency components associated with specific faults. In Case I, the generated inner race fault signals exhibit significant amplitudes at the ball-pass frequency of the inner race (BPFI) and its harmonics, which are consistent with the real signals. Similarly, in Case II, the complex modulation sidebands typical of gear teeth chipping are reproduced accurately. The cycle-consistency loss ensures that the background noise profile and the fundamental rotational frequency components from the source (normal) domain are preserved, while the CAC forces the generator to inject the distinct periodic impact features of the target fault class.
...
As illustrated in Fig. 6 (conf matrix at BR=0.05) and Fig. 7 (conf matrix at BR=1.0), the method is particularly effective at resolving confusion between "Gear Chipping" and "Mixed Wear." These faults often share similar sideband frequencies in the spectrum. The auxiliary classifier in our GAN model ensures that the generated signals emphasize the class-specific discriminative features, which standard GANs often overlook.
...
SMOTE \cite{smote} & 75.4 & 68.2 \\
ADASYN \cite{adasyn} & 76.8 & 70.1 \\
GAN \cite{gan} & 84.1 & 78.5 \\
ACGAN \cite{acgan} & 89.6 & 83.4 \\
WGAN-GP \cite{wgangp} & 92.3 & 87.8 \\
...
\subsection{Detailed Analysis of Experimental Setup}
...
\subsection{Signal-Level Evaluation of Generation Quality}
The visual results in Fig. 1 and Fig. 2 are complemented by a detailed analysis of the physical characteristics of the signals. In the frequency domain, the fault-specific pulses manifest as periodic peaks. For the Inner Race (IR) faults in Case I, these peaks appear at the BPFI of $5.4152 \times f_r$, where $f_r$ is the rotational frequency. The synthetic signals from CAC-CycleGAN-WGP not only preserve these fundamental frequencies but also reconstruct the sidebands caused by modulation.
...
\subsection{Multiclass Fault Diagnosis Performance - Case I}
...
Fig. 3 and Fig. 4 present the confusion matrices for Case I. In the $BR=0.005$ matrix (Fig. 3), we observe that the classifier is unable to distinguish between different fault types, often misclassifying Inner Race faults as Outer Race faults or failing to detect the fault entirely (misclassifying as Normal). This is because the majority class (Normal) and the limited minority samples (only 2 samples per fault class) do not provide a sufficiently rich feature manifold for the SVM to define robust boundaries. However, once the training set is augmented to $BR=1.0$ using the proposed method (Fig. 4), the diagonal elements of the confusion matrix show consistent values above $97\%$. This indicates that the generated samples effectively "fill in" the sparse regions of the fault manifolds, allowing the classifier to learn the nuances of each fault diameter and location.
...
\subsection{Multiclass Fault Diagnosis Performance - Case II}
...
Fig. 5 and Fig. 6 show the confusion matrices for Case II at $BR=0.05$ and $BR=1.0$, respectively. At $BR=0.05$ (Fig. 5), there is significant confusion between "Gear Chipping" and "Mixed Chipping+Wear" categories. These two faults share common modulation frequencies related to the gear meshing. When we augment the training set with our CRC-CycleGAN-WGP (Fig. 6), the SVM is able to better separate these complex combinations. This is because the auxiliary classifier (CAC) used during the generative phase forces the generator to emphasize the specific frequency bins that maximize class separability, which are often subtle in the original imbalanced set.

\subsubsection{Comparison of Augmentation Strategies}
Table \ref{tab:aug_comparison_case2} highlights the flexibility of our mounting strategy in Case II. We compare three distinct ways to reach $BR=1.0$: (1) oversampling existing real samples (standard oversampling), (2) generating samples with WGAN-GP, and (3) generating samples with CAC-CycleGAN-WGP.

\begin{table}[htbp]
\centering
\caption{Comparison of Augmentation Methods for Case II at $BR=1.0$}
\label{tab:aug_comparison_case2}
\begin{tabular}{lcc}
\hline
\textbf{Augmentation Way} & \textbf{Accuracy (\%)} & \textbf{PCC of Samples} \\ \hline
Oversampling (Real) & 71.3 & 1.000 \\
WGAN-GP (Synthetic) & 87.8 & 0.795 \\
CAC-CycleGAN-WGP (Synthetic) & \textbf{94.2} & \textbf{0.912} \\
\hline
\end{tabular}
\end{table}
...
\subsection{Benchmark Analysis against SOTA Methods}
...
For Case II, the performance gap is even wider. The proposed method leads other methods by at least $5.5\%$ in accuracy and $7\%$ in G-mean (as illustrated in Fig. 8). This suggests that as the industrial environment becomes noisier and the faults more complex, the structural constraints provided by CycleGAN become increasingly critical for valid signal synthesis.

\subsection{Exhaustive Ablation Study}
...
\subsection{Extended Diagnostic Performance Analysis: Case I}
...
We use the t-Distributed Stochastic Neighbor Embedding (t-SNE) to visualize the latent features extracted by the diagnostic model at different $BR$ values. In the highly imbalanced scenario ($BR=0.005$, Fig. 10), the features for the nine fault classes are heavily overlapping and compressed. The Normal class (majority) occupies a large portion of the feature space, effectively "pushing" the minority classes into a small, undifferentiated cluster. This visualization explains the low diagnostic accuracy (under 50\%): the classifier cannot find clear hyperplanes to separate the minority classes.

In contrast, after augmenting the training set to $BR=1.0$ using the proposed method (Fig. 11), the t-SNE plot shows that the ten classes have formed distinct, well-separated clusters. The synthetic samples are distributed throughout the manifold of each fault class, rather than forming isolated artifacts. This indicates that the CAC-CycleGAN-WGP has successfully learned the underlying signal distribution. The clusters for the same fault type with different diameters (e.g., OR 0.007, 0.014, 0.021) are positioned near each other but with clear boundaries between them. This level of granularity is essential for precision maintenance, where the severity of the fault directly influences the repair strategy.
...
\subsection{Extended Diagnostic Performance Analysis: Case II}
...
\subsection{Discussion of Comparative Experiments and Computational Cost}
As the comparisons in Table \ref{tab:comparison} and Table \ref{tab:aug_comparison_case2} show, the proposed CRC-CycleGAN-WGP is the most effective among all tested techniques. However, we must also consider the computational cost and the training complexity.
...
\subsection{Comprehensive Ablation Results}
...
In summary, the experimental results presented in this section provide strong empirical evidence for the efficacy of the CAC-CycleGAN-WGP framework. Across both bearing (Case I) and gearbox (Case II) datasets, the proposed method consistently superiorly outperforms traditional oversampling and standard generative approaches. We have demonstrated that by transforming numerous healthy signals into realistic faulty signals under cycle-consistency and auxiliary classification constraints, we can effectively mitigate the training bias caused by extremely imbalanced data. The visual and quantitative similarity analyses confirm that the synthetic signals are physically meaningful, while the multiclass and single-class diagnostic experiments show that the augmented training set enables the SVM classifier to achieve near-perfect accuracy even in cases where real fault samples are nearly non-existent. The robustness of the model to background noise and its effectiveness in complex mixed-fault scenarios further highlight its readiness for real-world industrial condition monitoring. This work paves the way for a more reliable and data-efficient approach to intelligent fault diagnosis.
